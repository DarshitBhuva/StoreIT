...
const ActionDropDown = ({ file }: { file: Models.Document }) => {
    ...
    const handleActions = async () => {
        ...
        const actions = {
            ...
            delete: () => deleteFile({fileId : file.$id, bucketField : file.bucketField, path})
                ...
                <DropdownMenuContent>
                    ...
                    {actionsDropdownItems.map((actionItem) => (
                        <DropdownMenuItem key={actionItem.value} className='shad-dropdown-item' onClick={() => {
                            ...
                            {actionItem.value === "download" ? <Link href={constructDownloadUrl(file.bucketField)} download={file.name} className='flex items-center gap-2'>
